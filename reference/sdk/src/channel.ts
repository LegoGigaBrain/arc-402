...
export class ChannelClient {
  ...
  async signStateUpdate(
    ...
    // signMessage wraps with \x19Ethereum Signed Message prefix — use _signTypedData / raw signing instead
    const sig = await this.signer.provider!.send("eth_sign", [
      await this.signer.getAddress(),
      ethers.hexlify(digest)
    ]).catch(async () => {
      // Fallback: sign the raw bytes without eth_sign prefix using signMessage on the digest bytes
